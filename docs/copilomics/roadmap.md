...
**Planning baseline: 6 September 2026.** This is a proposed delivery roadmap, not a statement that code has been implemented, published, deployed or scientifically qualified. Live [GitHub Issues](https://github.com/Tiliix/agentic-genomics-labs/issues) own acceptance criteria, dependencies and validation evidence. GitHub Project fields become canonical for workflow status and project metadata when a board is available; until then, record status explicitly in the live issues. Repository access alone does not establish project authorization, board availability or automation. This document deliberately contains no duplicate task-status table, deadlines or invented implementation owners.

...
- An **Azure subscription** with access to **Azure OpenAI** (a chat deployment,
  e.g. `gpt-4o`). Azure AI Search is **optional** (RAG citations).
...
4. **Configure secrets.** Copy the template and fill in your endpoint/keys:
   ```bash
   cp .env.example .env
   # edit .env: AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_DEPLOYMENT, etc.
   ```
